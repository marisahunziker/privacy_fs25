...
def train(model, train_loader, val_loader, test_loader,
    ...
    losses = torch.load(f'{losses_base_path}_epoch50.pt')
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1 + 50) >= 20 and (epoch + 1) % 5 == 0:
            checkpoint_model_path = f"{model_base_path}_epoch{epoch+1+50}.pt"
            checkpoint_losses_path = f"{losses_base_path}_epoch{epoch+1+50}.pt"
            checkpoint_metrics_path = f"{metrics_base_path}_epoch{epoch+1+50}.pt"
...
def main():
    ...
    for i, (learning_rate, weight_decay) in enumerate(experiments):
        ...
        model.load_state_dict(torch.load(f'{model_path}_epoch50.pt', map_location=device))
