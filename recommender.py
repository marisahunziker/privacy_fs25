...
def main(mode):
    ...
    if mode == 'np':
        print("Training with non-private optimizer...")
        model_path = 'models/model_np.pt'
        metrics_path = 'metrics/metrics_np.pt'
        losses_path = 'losses/losses_np.pt'

    elif mode == 'dp':
        print("Training with differentially private optimizer...")
        # Attach Opacus PrivacyEngine
        privacy_engine = PrivacyEngine()
        noise_multiplier = 1.1  # Adjust this value based on your needs
        max_grad_norm = 1.0  # L2 norm clipping
        ...
        model_path = f'models/model_dp_{noise_multiplier}_{max_grad_norm}.pt'
        metrics_path = f'metrics/metrics_dp_{noise_multiplier}_{max_grad_norm}.pt'
        losses_path = f'losses/losses_dp_{noise_multiplier}_{max_grad_norm}.pt'

    else:
        print("Invalid mode. Use 'np' or 'dp'.")
        return
    ...
    train(model, train_loader, val_loader, criterion, optimizer, model_path, losses_path, num_epochs=2)

    print("Evaluating model...")
    mse, mae, rmse = evaluate(model, test_loader, metrics_path)
    ...
    if mode == 'dp':
        ...
        print(f"DP epsilon after training: ε = {epsilon:.2f} (δ = 1e-5)")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        # Pass the first argument to the main function
        main(sys.argv[1])
    else:
        print("Please provide a mode as a command-line argument.")
